...
var log6 = createLogger("batch-queue");
...
`;
    ...
    log6.error("enqueue failed", { error: String(err) });
    throw err;
  }
}
function tryFlush() {
  try {
    ...
    log6.error("flush failed", { error: String(err) });
    return false;
  }
}
function flushQueue() {
...
`)) {
    try {
      ...
      log6.warn("skipping malformed queue line");
    }
  }
  if (events.length === 0)
    return;
  ...
  log6.info("batch flushed", { events: events.length });
}
...
var log7 = createLogger("proactive-retrieval");
...
[...truncated]`;
  ...
  saveState(sessionId, state);
  log7.info("proactive injection triggered", { sessionId, topic: currentTopic, results: results.length });
